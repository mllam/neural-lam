...
def validate_graph_directory(
    ...
    *grid* nodes, are *encoded* to *mesh* nodes, are *processed* on the mesh, and  # noqa: E501
    ...
    # enforced inside check_mesh_node_features below (dtype check)
    ...
    # enforced semantically and structurally by checking expected sender/receiver ranges  # noqa: E501
...
def cli(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Write the spec markdown to the given path. Idempotent: only writes if content differs from existing file. Exits 1 if the file was (re)written, 0 if no change.",  # noqa: E501
    )
    args = parser.parse_args(input_args)

    if args.write_spec_to_path:
        ...
        full_text = banner + spec_text
        path = Path(args.write_spec_to_path)
        if path.exists() and path.read_text() == full_text:
            return 0
        path.write_text(full_text)
        print(
            f"Spec file regenerated: {args.write_spec_to_path}. Stage the change and re-run.",  # noqa: E501
            file=sys.stderr,
        )
        return 1
