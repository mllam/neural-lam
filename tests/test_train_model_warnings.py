...
@pytest.mark.parametrize(
    ...
    [
        ...
        ([0], [], "Can not log train step 0"),
        ([-1], [], "Can not log train step -1"),
        ([], [0], "Can not log val step 0"),
    ],
)
def test_steps_to_log_validation(train_steps, val_steps, match_err):
    ...
    Steps below 1 previously passed this check silently, then wrapped
    around via ``time_step_loss[step - 1]`` in ForecasterModule and were
    logged under the wrong step's loss.
    """
    mock_args = MagicMock()
    mock_args.eval = None
    mock_args.load = None
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ([0], "Can not log validation step 0"),
        ([-1], "Can not log validation step -1"),
    ],
)
def test_var_leads_metrics_watch_step_validation(leads, match_err):
    """ValueError must be raised for --var_leads_metrics_watch steps outside
    [1, ar_steps_eval], same as for --val_steps_to_log."""
    mock_args = MagicMock()
    mock_args.eval = None
    mock_args.load = None
    mock_args.config_path = "dummy.yaml"
    mock_args.val_steps_to_log = []
    mock_args.train_steps_to_log = []
    mock_args.var_leads_metrics_watch = json.dumps({"0": leads})
    mock_args.ar_steps_eval = 10
    mock_args.ar_steps_train = 10
