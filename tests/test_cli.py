...
def test_train_model_defaults_to_using_all_ensemble_members(monkeypatch):
    captured = {}
...
def test_train_model_can_disable_all_ensemble_members(monkeypatch):
    captured = {}
    ...
    monkeypatch.setattr(
        neural_lam.train_model, "WeatherDataModule", DummyDataModule
    )
    monkeypatch.setitem(neural_lam.train_model.MODELS, "graph_lam", DummyModel)
    monkeypatch.setattr(
        neural_lam.train_model.utils,
    ...
    monkeypatch.setattr(
        neural_lam.train_model.seed,
        "seed_everything",
        lambda *args, **kwargs: None,
    )
    monkeypatch.setattr(
        neural_lam.train_model.torch.cuda,
        "is_available",
        lambda: False,
    )
    ...
    assert captured["use_all_ensemble_members"] is False
