...
- Load all ensemble members as independent samples by default for ensemble datastores in `WeatherDataset` / `WeatherDataModule` (with matching ensemble-member selection for forcing when available); single-member behavior now requires explicitly opting in via `--load_single_member`.
