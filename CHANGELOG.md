...
- Preserve filtered evaluation steps in `_test_spatial_steps` so `on_test_epoch_end` correctly associates spatial loss maps and saved PDF filenames when `val_steps_to_log` contains out-of-range steps [\#747](https://github.com/mllam/neural-lam/issues/747) @vikramsingh920
