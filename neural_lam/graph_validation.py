...
from .graph_generation_config import (
    GRAPH_CONFIG_FILENAME,
    GRAPH_CONFIG_SCHEMA_VERSION,
    REQUIRED_GRAPH_CONFIG_KEYS,
)
...
def _graph_config_metadata_errors(
    graph_dir_path: Path,
    *,
    require_graph_config: bool,
) -> list[GraphValidationError]:
    """Validate ``graph_config.json`` when present; optionally require the file."""
    cfg_path = graph_dir_path / GRAPH_CONFIG_FILENAME
    if not cfg_path.exists():
        if require_graph_config:
            return [
                GraphValidationError(
                    "graph_config: missing graph_config.json (required)",
                    file=GRAPH_CONFIG_FILENAME,
                )
            ]
        return []
    ...
    try:
        ...
        return [
            GraphValidationError(
                f"graph_config: invalid JSON: {exc}",
                file=GRAPH_CONFIG_FILENAME,
            )
        ]
    ...
    if not isinstance(data, dict):
        return [
            GraphValidationError(
                "graph_config: root must be a JSON object",
                file=GRAPH_CONFIG_FILENAME,
            )
        ]

    missing = sorted(REQUIRED_GRAPH_CONFIG_KEYS - data.keys())
    if missing:
        return [
            GraphValidationError(
                f"graph_config: missing required keys: {missing}",
                file=GRAPH_CONFIG_FILENAME,
            )
        ]

    sch = data.get("schema_version")
    if sch != GRAPH_CONFIG_SCHEMA_VERSION:
        return [
            GraphValidationError(
                f"graph_config: unsupported schema_version={sch!r}, "
                f"expected {GRAPH_CONFIG_SCHEMA_VERSION}",
                file=GRAPH_CONFIG_FILENAME,
            )
        ]
...
def validate_graph_dir(
    ...
    *,
    ...
    require_graph_config: bool = False,
) -> list[GraphValidationError]:
    """
    Validate a graph directory produced by neural-lam graph generation.

    Structural checks (tensor shapes, edge counts, required files) always run.
    When ``graph_config.json`` exists, it is checked for valid JSON and required
    keys. Set ``require_graph_config=True`` to fail if the file is missing.
    ...
    errors += _graph_config_metadata_errors(
        graph_dir_path,
        require_graph_config=require_graph_config,
    )
    return errors
