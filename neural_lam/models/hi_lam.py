...
from neural_lam.interaction_net import MeshInitNet, MeshDownNet, InteractionNet
...
class HiLAM(BaseHiGraphModel):
    ...
    def __init__(self, args, init_device):
        ...
        self.mesh_down_gnns = nn.ModuleList([self.make_down_gnns() for _ in
            range(args.processor_layers)]) # Nested lists (proc_steps, N_levels-1)
        self.mesh_down_same_gnns = nn.ModuleList([self.make_same_gnns() for _ in
            range(args.processor_layers)]) # Nested lists (proc_steps, N_levels)

        # Make up GNNs, both for up edges and same level
        self.mesh_up_gnns = nn.ModuleList([self.make_up_gnns() for _ in
            range(args.processor_layers)]) # Nested lists (proc_steps, N_levels-1)
        self.mesh_up_same_gnns = nn.ModuleList([self.make_same_gnns() for _ in
            range(args.processor_layers)]) # Nested lists (proc_steps, N_levels)
    ...
    def mesh_down_step(self, mesh_rep_levels, mesh_same_rep, mesh_down_rep, down_gnns,
        ...
        for level_l, down_gnn, same_gnn in zip(
            ...
            node_reps = torch.cat((mesh_rep_levels[level_l], mesh_rep_levels[level_l+1]),
    ...
    def mesh_up_step(self, mesh_rep_levels, mesh_same_rep, mesh_up_rep, up_gnns,
        ...
        for level_l, (up_gnn, same_gnn) in enumerate(zip(up_gnns, same_gnns[1:]),
                start=1):
            # Extract representations
            node_reps = torch.cat((mesh_rep_levels[level_l-1], mesh_rep_levels[level_l]),
