...
class WeatherDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        static_features = torch.tensor(np.load(water_path),
                dtype=torch.float32).unsqueeze(-1) # (N_x, N_y, 1)
        # Flatten
        static_features = static_features.flatten(0,1) # (N_grid, 1)

        # === Forcing features ===
        # Forcing features
        flux_path = os.path.join(self.sample_dir_path,
                f"nwp_toa_downwelling_shortwave_flux_{sample_datetime}.npy")
        flux = torch.tensor(np.load(flux_path),
                dtype=torch.float32).unsqueeze(-1) # (N_t', N_x, N_y, 1)
        ...
        year_angle = (second_into_year/constants.seconds_in_year
                )*2*torch.pi # (sample_len,)
